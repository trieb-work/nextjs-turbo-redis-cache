...
KEY_PREFIX, VERCEL_URL, VERCEL_ENV are optional. KEY_PREFIX is used first to create a key prefix for the redis keys. If it is not set, VERCEL_URL is used. VERCEL_ENV is used to determine the database to use. Only VERCEL_ENV=production will show up in DB 0 (redis default db). All other values of VERCEL_ENV will use DB 1, use `redis-cli -n 1` to connect to different DB 1. This is another protection feature to avoid that different environments (e.g. staging and production) will overwrite each other.
...
| keyPrefix                     | Prefix added to all Redis keys                                                                                                                             | `process.env.KEY_PREFIX \|\| process.env.VERCEL_URL \|\| 'UNDEFINED_URL_'`                                                                                    |
