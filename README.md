...
- options.keyPrefix → KEY*PREFIX → BUILD_ID (from `.next/BUILD_ID`) → VERCEL_URL → `UNDEFINED_URL*`
...
| keyPrefix                     | Prefix added to all Redis keys                                                                                                                             | `RedisStringsHandler` default: `process.env.KEY_PREFIX \|\| process.env.VERCEL_URL \|\| 'UNDEFINED_URL_'`<br> Next handlers resolve: `options.keyPrefix \|\| KEY_PREFIX \|\| BUILD_ID \|\| VERCEL_URL \|\| 'UNDEFINED_URL_'` |
