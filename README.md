...
| **`updateTags(tags, durations)`** | Always persists `Date.now()` in Redis so `getExpiration()` marks entries stale immediately (survives restarts). `expire > 0` (e.g. `revalidateTag(tag, 'max')`) keeps Redis entries so stale content can be served (SWR). `expire: 0` or omitted hard-deletes matching keys. |
