...
export default class RedisStringsHandler implements CacheHandler {
    ...
    private redisDeduplicationHandler: DeduplicatedRequestHandler<Client["get"], string | Buffer | null>;
    ...
    public async get(key: GetParams[0], ctx: GetParams[1]) {
        ...
        const cacheValue = JSON.parse(result) as (CacheHandlerValue & { lastModified: number }) | null;
        ...
        const combinedTags = new Set([...(ctx?.softTags || []), ...(ctx?.tags || [])]);
        ...
        for (const tag of combinedTags) {
            ...
            if (
                revalidationTime &&
                revalidationTime > cacheValue.lastModified
            ) {
    ...
    public async set(key: SetParams[0], data: SetParams[1] & { lastModified: number }, ctx: SetParams[2]) {
        if (data.kind === "FETCH") {
        ...
        const expireAt = ctx.revalidate && Number.isSafeInteger(ctx.revalidate) && ctx.revalidate > 0 ?
            this.estimateExpireAge(ctx.revalidate) : this.estimateExpireAge(this.defaultStaleAge)
        ...
        if (ctx.tags && ctx.tags.length > 0) {
            ...
            if (!currentIsSameAsNew) {
                setTagsOperation = this.sharedTagsMap.set(key, structuredClone(ctx.tags) as string[]);
            }
        }
