...
// The cache entry key of a nested fetch request is a hash generated by Next.js
// based on headers/payload etc. Next.js changed this hash algorithm in 16.3.0,
// so the expected hash differs per Next.js major/minor. Resolve it from the
// test app's installed Next.js version so the same suite works across versions.
function readNextVersion(appDir: string): number[] {
  const pkg = JSON.parse(
    readFileSync(join(appDir, 'package.json'), 'utf8'),
  ) as { dependencies: { next?: string } };
  const raw = (pkg.dependencies.next || '0.0.0').replace(/[^\d.]/g, '');
  return raw.split('.').map((p) => Number(p) || 0);
}

function gte(version: number[], target: number[]): boolean {
      ...
      describe('should cache the nested fetch request (but not the API route itself)', () => {
        ...
        if (process.env.SKIP_OPTIONAL_LONG_RUNNER_TESTS !== 'true') {
          ...
          it('After the new request was made the redis key and hashmap should be set again', async () => {
            ...
            const cacheEntryKey = NESTED_FETCH_API_ROUTE_CACHE_KEY;
    ...
    describe('With a cached revalidation fetch request inside a page', () => {
      ...
      it('should set all cache entries for this page after request is finished', async () => {
        ...
        const keys2 = await redisClient.keys(
          process.env.VERCEL_URL + NESTED_FETCH_PAGE_CACHE_KEY,
        );
