...
async function cfPurgePrefix(prefix: string) {
  const zoneId = process.env.CF_ZONE_ID;
  const token = process.env.CF_API_TOKEN;
  const hostname = process.env.CF_TUNNEL_HOSTNAME;
  if (!zoneId || !token || !hostname) return;
  await fetch(
    `https://api.cloudflare.com/client/v4/zones/${zoneId}/purge_cache`,
    {
      ...
      body: JSON.stringify({ prefixes: [`${hostname}${prefix}`] }),
    },
  );
}
...
async function doUpdateTag() {
  ...
  await cfPurgePrefix('/cache-lab/tag-invalidation');
}
    ...
    <main className="mx-auto max-w-3xl p-10">
      ...
      <header className="mb-6">
        ...
        <p className="mt-2 text-sm text-slate-600">
          ...
          <code>cacheTag('cache-lab:tag')</code>. Buttons invalidate both Redis
          and the Cloudflare CDN cache.
        </p>
      </header>
      ...
      <div className="mt-6 rounded-lg border bg-slate-50 p-5 text-sm text-slate-700">
        <div className="space-y-2">
          ...
          <p>
            <strong>updateTag</strong> — expires + immediately refreshes Redis
            cache, then purges CF prefix so the CDN also sees a fresh response.
          </p>
          <p>
            <strong>revalidateTag</strong> — invalidates Redis with
            stale-while-revalidate behavior, then purges CF prefix.
          </p>
          <p>
